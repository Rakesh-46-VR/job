...
export function JobDetailsModal({ job, isOpen, onClose, onApply }: JobDetailsModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-background w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg shadow-xl border border-border m-4 animate-in zoom-in-95 duration-200 flex flex-col">
                {/* Header */}
                <div className="p-6 border-b border-border sticky top-0 bg-background z-10 flex justify-between items-start">
                    <div>
                        <h2 className="text-2xl font-serif font-bold text-foreground">{job.title}</h2>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {job.skills.map(skill => (
                                <Badge key={skill} variant="default" className="bg-secondary/50 text-foreground hover:bg-secondary border-transparent">
                                    {skill}
                                </Badge>
                            ))}
                        </div>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-border bg-secondary/5 sticky bottom-0 flex justify-end gap-3">
